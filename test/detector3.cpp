...
int main(int argc, char **argv){
    ...
    for (int i = 0; i < imgPaths.size(); i++) {
        ...
        Mat satMask;
        threshold(sat, satMask, 40, 255, THRESH_BINARY_INV | THRESH_OTSU);
        threshold(sat, satMask, 120, 255, THRESH_BINARY);

        Mat dif;
        threshold(sat, dif, 120, 255, THRESH_BINARY_INV);
        ...
        if (maxAVG > 0.43) cout << "\tSECONDO! Max avg: " <<  maxAVG << "\n\n";
